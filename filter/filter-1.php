...
// Create container for filters v.4.2
function add_container_for_filters() {
    ...
    if (is_shop() || is_product_category() || is_product_tag() || is_search()) {
        ...
        if ($query->have_posts()) {
            ...
            $available_tags = wp_get_object_terms($product_ids, 'product_tag', array('hide_empty' => true));
            $available_colors = wp_get_object_terms($product_ids, 'pa_czvet', array('hide_empty' => true));
            $available_sizes = wp_get_object_terms($product_ids, 'pa_razmer', array('hide_empty' => true));
            $available_materials = wp_get_object_terms($product_ids, 'pa_material', array('hide_empty' => true));
            $available_heights = wp_get_object_terms($product_ids, 'pa_vysota', array('hide_empty' => true));
            $available_heights_protect = wp_get_object_terms($product_ids, 'pa_vysota-zashhity', array('hide_empty' => true));
            $available_brend = wp_get_object_terms($product_ids, 'pa_brend', array('hide_empty' => true));
            $available_categories = wp_get_object_terms($product_ids, 'product_cat', array('hide_empty' => true));

            // Check for stock statuses
            $stock_statuses = array();
            $stock_status_query_args = array(
                'post_type' => 'product',
                'post_status' => 'publish',
                'fields' => 'ids',
                'meta_query' => array(
            ...
            $stock_query = new WP_Query($stock_status_query_args);

            if ($stock_query->have_posts()) {
                foreach ($stock_query->posts as $product_id) {
                    $stock_status = get_post_meta($product_id, '_stock_status', true);
                    if ($stock_status == 'instock') {
                        $stock_statuses[$stock_status] = 'В наличии';
                    }
                }
            }
        }
        ...
        //             echo '<label class=""><input type="checkbox" name="tag[]" value="' . esc_attr($tag->slug) . '" ' . $checked . '><span>' . esc_html($tag->name) . '</span></label>';
        //         }
        //     }
        //     echo '</div>';
        // }     
    ...
    if (is_shop() && strpos($_SERVER['REQUEST_URI'], '/shop/') !== false && strlen($_SERVER['REQUEST_URI']) == strlen('/shop/')) {
        // Create container for backorder filters v.3.9
        echo '<div class="filters_2024_backorder">';
        // echo '<h2>Предзаказ</h2>';
...
function redirect_filter_to_shop() {
    ...
    $filters = ['tag', 'czvet', 'razmer', 'material', 'vysota', 'vysota-zashhity', 'brend', 'stock_status', 'category'];

    // Check if any filter is present in the query string
    $has_filters = false;
    $query_args = [];
    ...
    // If filters exist but `/shop/` is missing, redirect
...
function redirect_if_no_filtered_products_with_alert() {
    if (is_shop() && !is_admin() && (isset($_GET['czvet']) || isset($_GET['razmer']) || isset($_GET['tag']) || isset($_GET['material']) || isset($_GET['vysota']) || isset($_GET['category']) || isset($_GET['stock_status']))) {
        $args = array(
            'post_type' => 'product',
            'posts_per_page' => 1, // We only need to check if there's at least one product
            'tax_query' => array('relation' => 'AND')
        );
...
function no_products_alert_and_back_js() {
    ...
    <script type="text/javascript">
        ...
        setTimeout(() => {
            // window.history.back();
            // alert("По данному запросу товаров не найдено");
...
// reset button v.3.9
// Add the JS function to handle reset button action
function add_backorder_filter_reset_script() {
    ?>
    <script type="text/javascript">
...
add_action('wp_footer', 'add_backorder_filter_reset_script', 170);


// Create container for backorder filters v.3.9
// function add_container_for_backorder() {
    
// }
// add_action('woocommerce_before_shop_loop', 'add_container_for_backorder', 30);

// backorder filters process v.3.9
...
function filter_products_by_backorder($q) {
    if ((is_shop() || is_product_category()) && isset($_GET['backorder']) && $_GET['backorder'] == '1') {
        ...
        if (!empty($product_ids)) {
            ...
            // If no products found, ensure no products are displayed
...
function filter_s32_enqueue_scripts() {
	...
	wp_enqueue_script(
		'filter-js', // Handle for the JS file
	...
	// wp_enqueue_style(
	// 	'custom-stock-data-css', // Handle for the CSS file
	// 	plugin_dir_url(__FILE__) . 'var-stock-data.css',
	// 	array(), // Dependencies, if any
	// 	'1.0.0' // Version number
	// );
}
